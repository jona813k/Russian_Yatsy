...
// ---------------------------------------------------------------------------
// Colour palette (classic light theme — separate from the RPG gladiator theme)
// ---------------------------------------------------------------------------
const C = {
  bg: '#f5f5f0',
  white: '#ffffff',
...
// ---------------------------------------------------------------------------
// Hook — API calls for the classic game
// ---------------------------------------------------------------------------
function useGame() {
  const [state, setState] = useState(null);
  const [lastResult, setLastResult] = useState(null);
  const [failedRoll, setFailedRoll] = useState(null);
  ...
  return { state, lastResult, failedRoll, loading, error, startGame, selectNumber, skipTurn };
}
  ...
  return (
    <div>
      <div style={{
        ...
        border: '1px solid #f0d060', marginBottom: 12,
      }}>
    ...
    <div>
      ...
      <div style={{ marginTop: 14, minHeight: 44 }}>
        {derivedTarget != null && !disabled && (
          <button onClick={() => onSelect(derivedTarget)} style={{
...
export default function ClassicGame() {
  const { state, lastResult, failedRoll, loading, error, startGame, selectNumber, skipTurn } = useGame();
